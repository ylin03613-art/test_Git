...
static  u8  s_i1msFlag  = FALSE;    //将1ms标志位的值设置为FALSE
...
{
  static  i32 s_iCnt100000  = 0;                        //定义一个静态变量s_iCnt1000作为1s计数器
  ...
  s_iCnt100000++;           //1000ms计数器的计数值加1
  
  if(s_iCnt100000 >= 100000)  //1000ms计数器的计数值大于或等于1000
  {                                                   
    s_iCnt100000 = 0;       //重置1000ms计数器的计数值为0
...
void InitTimer(void)
{
  ...
  ConfigTimer5(9, 71);  //72MHz/(71+1)=1MHz，由0计数到9为10us
}
