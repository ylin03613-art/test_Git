...
*                                              内部变量
*********************************************************************************************************/
 u16 s_arrADC1Data[2];   //存放ADC转换结果数据
 StructU16CirQue  s_structADCCirQue;            //ADC循环队列
 u16              s_arrADCBuf[ADC1_BUF_SIZE];   //ADC循环队列的缓冲区
...
{                          
  ...
  ADC_RegularChannelConfig(ADC1, ADC_Channel_1, 1, ADC_SampleTime_55Cycles5); //设置采样时间为55.5个周期
  ADC_RegularChannelConfig(ADC1, ADC_Channel_17, 2, ADC_SampleTime_55Cycles5); //内部参照电压
